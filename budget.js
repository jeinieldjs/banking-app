const transactionsStorage = JSON.parse(localStorage.getItem("transactions"));
...
if (loggedClient) {
  ...
  function updateLocalStorage() {
    let clientIndex = clients.findIndex(
      (client) => client.name === loggedInUser
    );
    if (clientIndex !== -1) {
  ...
  function withdraw() {
    ...
    withdrawBtn.addEventListener("click", function () {
      ...
      const confirmation = confirm(
        "Are you sure you want to withdraw " + withdrawAmount + "PHP ?"
      );
      if (!confirmation) {
        return;
      }
      ...
      const newTransaction = {
        type: "Withdraw",
        user: loggedClient.name,
        date: new Date().toLocaleString(),
        amount: withdrawAmount,
      };

      const updatedTransactions = [...transactionsStorage, newTransaction];
  ...
  function deposit() {
    ...
    depositBtn.addEventListener("click", function () {
      ...
      const confirmation = confirm(
        "Are you sure you want to deposit " + depositAmount + "PHP ?"
      );
      if (!confirmation) {
        return;
      }
  ...
  function transfer() {
    ...
    transferBtn.addEventListener("click", function () {
      ...
      const confirmation = confirm(
        `Are you sure you want to transfer ${transferAmount} PHP to ${recipient}?`
      );
      if (!confirmation) {
        return;
      }
      ...
      const newTransaction = {
        type: "Transfer",
        sender: loggedClient.name,
        receiver: recipientClient.name,
        date: new Date().toLocaleString(),
